...
class Dali_movie():
    ...
    def title(self, text, backgroundColor="black", duration=5, textColor = "black"):
        if backgroundColor == "black":
            textColor = "white"
        return TextClip(self._font_path, font_size=20, text=text, bg_color=backgroundColor, duration=duration, color=textColor)
    ...
    def add(self, media, mode=None, offset=None, reference=None):
        if not mode:
            return self.add_append(media)
        if mode == Placement_Mode.AFTER and reference:
            return self.add_after(media, offset, reference)
        
        if mode == Placement_Mode.FROM and reference:
            return self.add_from(media, offset, reference)

        return False

    def add_append(self, media):
    ...
    def add_after(self, media, offset, reference):
        if offset == None: 
            offset = 0
        track = self._get_track(media)
        index, reference_dali_clip, following_dali_clip = self._get_clip(track, reference)
        
        
        
    def add_from(self, media, offset, reference):
        if offset == None: 
            offset = 0
    ...
    def _get_clip(self, track, reference):
        try:
